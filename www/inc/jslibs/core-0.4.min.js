...
{$('.tool-numbers').removeClass('tool-numbers-on');editor.setOption('lineNumbers',false);}});$('#tool-fullscreen').bind('click',function(){var checked=$('#tool-fullscreen').is(':checked');if(checked==1){$('.tool-fullscreen').addClass('tool-fullscreen-on');$('#holder').css('width','100%');}else
{$('.tool-fullscreen').removeClass('tool-fullscreen-on');$('#holder').css('width','');}});enableHover();});var timeDiff={time:undefined,setStartTime:function(){d=new Date();this.time=d.getTime();},getDiff:function(){d=new Date();return d.getTime()-this.time;}};function stringBreak(str,col){var result='';for(var i=0;i<str.length;i++){result+=str.charAt(i);if(((i+1)%col==0)&&(0<i)){result+="\n";}}return result;}function generateKey(){var index='abcdefghijklmnopqrstuvwxyz0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ';var key='';for(var i=1;i<25;i++){key+=index[Math.floor(Math.random()*index.length)]};return key;}function enableHover(){$('#en').hover(function(){$('#result').show();$('#encrypttime').show();$('#result').focus();},function(){$('#result').hide();$('#encrypttime').hide();$('#text').focus();});}function onCodeChange(ed,obj){__timer_coloring=timeDiff.getDiff();$('#coloring').html('syntax: '+__timer_coloring+'ms,');__timer_total=(__timer_decryption+__timer_coloring);$('#totaltime').html('total: '+__timer_total+'ms');}function decrypt(cipher,data){$('#decrypting').show();$('#insertkey').hide();timeDiff.setStartTime();var output=ez.aes.decrypt(cipher,data);__timer_decryption=timeDiff.getDiff();timeDiff.setStartTime();$('#execute').html('decryption: '+__timer_decryption+'ms,');$('.cm-s-default').parent().show();$('#decrypting').hide();return output;}function encrypt(cipher,data){timeDiff.setStartTime();var encrypt=stringBreak(ez.aes.encrypt(cipher,data),96);__timer_encryption=timeDiff.getDiff();$('#encrypttime').html('encryption: '+__timer_encryption+'ms');return encrypt;}function requestData(password){var url=window.location.href;var hash=window.location.hash;var index_of_hash=url.indexOf(hash)||url.length;var hashless_url=url.substr(0,index_of_hash);$.ajax({url:hashless_url,type:'POST',dataType:'json',data:'&p='+password,cache:false,success:function(json){$('#data').val(json.data);$('#syntax').val(json.syntax);editor.setOption('mode',$('#syntax').val());if(hash==''){$('#askpassword').hide();$('#insertkey').show();$('#typekey').focus();}else
{$('#askpassword').hide();editor.setValue(decrypt(hash.substring(1),$('#data').val()));}},error:function(){alert('bad password!');}});}function submitData(url){if($('#text').val()==''){return false;}else if(_encrypting==null&&$('#result').val()==''&&$('#text').val()!=''){ezcrypt(lib,function(){ez=this;$('#result').val(encrypt($('#key').val(),$('#text').val()));});}else if(_encrypting!=null&&$('#text').val()!=''){setTimeout('submitData()',100);return false;}$('#en').unbind('mouseenter mouseleave');$('#result').show();$('#encrypttime').show();var data=$('#result').val();data=encodeURIComponent(data);var password='';if($('#usepassword').is(':checked')){password=ez.sha($('#typepassword').val());}var ttl=$('#ttl option:selected').val();var syntax=$('#syntax option:selected').val();if(typeof(syntax)=='undefined'){syntax=$('#syntax').val();}$.ajax({url:url,type:'POST',dataType:'json',data:'&data='+data+'&p='+password+'&ttl='+ttl+'&syn='+syntax,cache:false,success:function(json){if(ttl==-100){}else
{window.location=url+((password!='')?'p/':'')+json.id+'#'+$('#key').val();}},error:function(){enableHover();alert('error submitting form');}});}