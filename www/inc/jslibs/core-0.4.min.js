...
{$('#askpassword').hide();editor.setValue(decrypt(hash.substring(1),$('#data').val()));}},error:function(){alert('bad password!');}});}function submitData(){if($('#text').val()==''){return false;}else if(_encrypting==null&&$('#result').val()==''&&$('#text').val()!=''){ezcrypt(lib,function(){ez=this;$('#result').val(encrypt($('#key').val(),$('#text').val()));});}else if(_encrypting!=null&&$('#text').val()!=''){setTimeout('submitData()',100);return false;}$('#en').unbind('mouseenter mouseleave');$('#result').show();$('#encrypttime').show();var data=$('#result').val();data=encodeURIComponent(data);var password='';if($('#usepassword').is(':checked')){password=ez.sha($('#typepassword').val());}var ttl=$('#ttl option:selected').val();var syntax=$('#syntax option:selected').val();if(typeof(syntax)=='undefined'){syntax=$('#syntax').val();}$.ajax({url:'/',type:'POST',dataType:'json',data:'&data='+data+'&p='+password+'&ttl='+ttl+'&syn='+syntax,cache:false,success:function(json){if(ttl==-100){}else
{window.location='/'+((password!='')?'p/':'')+json.id+'#'+$('#key').val();}},error:function(){enableHover();alert('error submitting form');}});}